...
public class TesteServicoCliente {
	...
	public void teste01_inserirSemErro() throws Exception {
		...
		ss.insert(o);
		Cliente aux = (Cliente) ss.findByName("Victor").get(0);
		assertNotNull(aux);
	...
	try {
		Cliente o = new Cliente(1L, "Victo@r", "picinininho", "senha", "victorpici", "09535074679", "(11)9823-5881",
				"victorpicinin@gmail.com", new Date(), new Date());
	ss.insert(o);
	} catch (Exception e){
	...
	public void teste03_atualizar() throws Exception {
		log.info("============> Iniciando o teste " + Thread.currentThread().getStackTrace()[1].getMethodName());
		Cliente o = (Cliente) ss.findByName("Victor").get(0);
		o.setNome("Victor modificado");
		ss.update(o);
		Cliente aux = (Cliente) ss.findByName("Victor modificado").get(0);
		assertNotNull(aux);
		log.info("============> Finalizando o teste " + Thread.currentThread().getStackTrace()[1].getMethodName());
	}
	...
	public void teste04_excluir() throws Exception {
		...
		Cliente o = (Cliente) ss.findByName("Victor").get(0);
		ss.delete(o);
		assertEquals(0, ss.findByName("Victor modificado").size());
		log.info("============> Finalizando o teste " + Thread.currentThread().getStackTrace()[1].getMethodName());
	}
